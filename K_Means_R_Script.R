...
# from player_season_stats stats , team_salary tp 
...
# select tp.player_id,PLAYER_SEASON_STATS_age,p.experience,p.height,p.weight, salary, PLAYER_SEASON_STATS_FG, PLAYER_SEASON_STATS_FGA,PLAYER_SEASON_STATS_FGP, PLAYER_SEASON_STATS_DRB,PLAYER_SEASON_STATS_PTS,PLAYER_SEASON_STATS_FT,PLAYER_SEASON_STATS_FTP
...
salary_player<-read.table("/Users/vinutha/IS5126/GP2/csv/player_stat_kmeans.csv",header=T,sep=",")
# player_sal_stat<-read.table("/Users/vinutha/Desktop/player_stat_new.csv",header=T,sep=",")
######################################################################################################
...
######################################################################################################
#K Means Plot: POINTS Vs Salary
sal_age<-subset(salary_player,select=c(AGE,SALARY))
x<-data.matrix(sal_age,rownames.force = NA)
(cl=kmeans(na.omit(x),5))
...
sal_exp<-subset(salary_player,select=c(EXPERIENCE,SALARY))
